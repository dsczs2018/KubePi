...
build_web_kubepi:
	cd $(KUBEPIDIR) && npm config set registry https://registry.npm.taobao.org && npx browserslist@latest --update-db && npm install && npm run-script build
build_web_dashboard:
	cd $(DASHBOARDDIR) && npm config set registry https://registry.npm.taobao.org && npx browserslist@latest --update-db && npm install && npm run-script build
build_web_terminal:
	cd $(TERMINALDIR) && npm config set registry https://registry.npm.taobao.org && npx browserslist@latest --update-db && npm install && npm run-script build
